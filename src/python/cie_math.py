...
class _MetricAccumulator:
    ...
    _current_run: int = 0
    # Declared explicitly: with slots=True an attribute assigned only in
    # __post_init__ raises AttributeError, which made this whole module
    # unusable before it was wired up.
    histogram: Any = field(init=False, repr=False)
    ...
    def _update_numpy(self, chunk: bytes) -> None:
        ...
        counts = np.bincount(view, minlength=256).astype(self.histogram.dtype, copy=False)
